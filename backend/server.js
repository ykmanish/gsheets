...
const MCP_ALLOWED_COLLECTIONS = new Set(
  (process.env.MCP_ALLOWED_COLLECTIONS || [
    ...
    "employeeExecutiveReportAnalyses",
    "employeeExecutiveReportAnswers",
    "employeeReportSubmissions",
    "employeeSiteTaskRemarks",
    "dmrDailySnapshots",
    "dmrWhatsAppReminderRuns",
    "formSubmissions",
    "forms",
    "forumConversations",
    "forumMessages",
...
function receiveWhatsAppWebhook(req, res) {
  try {
    const result = whatsappService.handleWebhook(req.body, req.rawBody, req.headers["x-hub-signature-256"]);
    if (result?.received?.length) {
      addActivityLog({
        action: "Received WhatsApp webhook",
        target: `${result.received.length} message${result.received.length === 1 ? "" : "s"}`,
        category: "whatsapp",
        details: {
          messages: result.received.map((message) => ({
...
app.post("/api/webhooks/whatsapp", receiveWhatsAppWebhook);
...
app.post("/forum/refine-message", async (req, res) => {
  try {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system: [
          ...
          "Improve clarity, grammar, warmth, and readability without making it formal unless the original asks for that tone.",
          "Do not add facts, greetings, signatures, markdown, quotes, or explanations.",
    ...
    res.status(500).json({ error: error.message || "Could not refine message" });
  }
});
    ...
    for (let rowIndex = 1; rowIndex < values.length; rowIndex += 1) {
      ...
      for (const column of categoryColumns) {
        ...
        allRecords.push({
          ...
          timeliness: submittedAt?.minutes !== null && submittedAt?.minutes !== undefined && submittedAt.minutes <= 930 ? "on-time" : "delayed",
...
function projectGanttRows(project = {}) {
  ...
  [...phases].sort(scheduleSort).forEach((phase, phaseIndex) => {
    ...
    rows.push({
      ...
      title: projectText(phase.name) || `Phase ${phaseIndex + 1}`,
      status: projectText(phase.status) || "todo",
      startDate,
      dueDate,
      duration: dayCount(startDate, dueDate),
    ...
    phaseTasks.forEach((task) => {
      const taskStart = projectDateKey(task.startDate) || projectDateKey(task.dueDate) || startDate;
      const taskDue = projectDateKey(task.dueDate) || taskStart;
      const subtasks = Array.isArray(task.subtasks) ? task.subtasks : [];
      const doneSubtasks = subtasks.filter((subtask) => subtask.done).length;
      rows.push({
        type: "task",
        id: task.id,
        title: projectText(task.title) || "Untitled task",
        status: (task.blockedBy || []).length ? "blocked" : projectText(task.status) || "todo",
        priority: projectText(task.priority) || "medium",
        startDate: taskStart,
        dueDate: taskDue,
...
function generateProjectGanttPdf(project = {}, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const endKey = dates.at(-1) || startKey;
    const addProjectDays = (date, amount) => {
    ...
    const chipWidth = (text, fontSize = 5.8, pad = 9, max = 90) => {
      doc.font(dmrPdfFonts.bold).fontSize(fontSize);
      return Math.min(max, Math.max(20, doc.widthOfString(text) + pad));
    };
    const writeBadge = (x, y, w, text, bg, fg, fontSize = 5.8) => {
      doc.roundedRect(x, y, w, 12, 4).fill(bg);
    ...
    for (let unitStartIndex = 0; unitStartIndex < timelineUnits.length; unitStartIndex += unitsPerPage) {
      ...
      for (let rowStart = 0; rowStart < Math.max(1, rows.length); rowStart += rowsPerPage) {
        ...
        pageNo += 1;
        const visibleRows = rows.slice(rowStart, rowStart + rowsPerPage);
        ...
        doc.fillColor("#5f665c").font(dmrPdfFonts.bold).fontSize(5.8).text("DUR", page.left + taskW + dateW * 2, headerY + 5, { width: durationW, align: "center" });
    ...
    if (!rows.length) {
      doc.fillColor("#858b82").font(dmrPdfFonts.regular).fontSize(11).text("Add start dates or deadlines to phases and tasks to build the Gantt chart.", 0, 260, { align: "center" });
    }
    doc.end();
  });
}
...
app.get("/project-dashboard/projects/:id/gantt/pdf", async (req, res) => {
  try {
    ...
    console.error("Project Gantt PDF error:", error);
    res.status(500).json({ error: `Could not generate Gantt PDF: ${error.message}` });
  }
});
...
const MRN_WHATSAPP_WATCHER_BASELINE_VERSION = "2026-07-28T13:40:00+05:30";
let mrnWhatsappWatcherTimer = null;
let mrnWhatsappWatcherRunning = false;
const mrnWhatsappRuntimeProcessedKeys = new Set();
...
async function getMrnWhatsappSettings() {
  const db = await connectAuthDb();
  const saved = await db.collection("platformSettings").findOne({ _id: MRN_WHATSAPP_SETTINGS_ID });
  return {
    ...MRN_WHATSAPP_DEFAULTS,
    ...(saved?.settings || {}),
    templates: {
      ...MRN_WHATSAPP_DEFAULTS.templates,
...
async function saveMrnWhatsappSettings(input = {}) {
  const current = await getMrnWhatsappSettings();
  ...
  const settings = {
    ...
    templates: {
      actionRequest: projectText(input.templates?.actionRequest) || MRN_WHATSAPP_DEFAULTS.templates.actionRequest,
      approved: projectText(input.templates?.approved) || MRN_WHATSAPP_DEFAULTS.templates.approved,
      declined: projectText(input.templates?.declined) || MRN_WHATSAPP_DEFAULTS.templates.declined,
      comment: projectText(input.templates?.comment) || MRN_WHATSAPP_DEFAULTS.templates.comment,
    },
    languages: {
      actionRequest: projectText(input.languages?.actionRequest) || MRN_WHATSAPP_DEFAULTS.languages.actionRequest,
      approved: projectText(input.languages?.approved) || MRN_WHATSAPP_DEFAULTS.languages.approved,
  ...
  await db.collection("platformSettings").updateOne(
    ...
    { $set: { _id: MRN_WHATSAPP_SETTINGS_ID, settings } },
    { upsert: true },
  );
  return settings;
}

async function recordMrnWhatsappLastRun(run = {}) {
  const db = await connectAuthDb();
  const lastRun = {
...
async function markMrnWhatsappActionRequestProcessed(mrnNo, rowNumber = 0, spreadsheetId = "") {
  ...
  if (!key) return;
  mrnWhatsappRuntimeProcessedKeys.add(key);
  if (mrnWhatsappRuntimeProcessedKeys.size > 2000) {
    const oldest = mrnWhatsappRuntimeProcessedKeys.values().next().value;
    if (oldest) mrnWhatsappRuntimeProcessedKeys.delete(oldest);
  }
  const db = await connectAuthDb();
  const update = {
...
async function initializeMrnWhatsappWatcherFromSheet(records = [], spreadsheetId = "") {
  const maxRowNumber = records.reduce((max, row) => Math.max(max, Number(row.rowNumber) || 0), 0);
  const latestRowNumber = Number(records[records.length - 1]?.rowNumber) || 0;
  const baselineRowNumber = latestRowNumber > 2 ? latestRowNumber - 1 : maxRowNumber;
  const baselineRecords = records.filter((row) => (Number(row.rowNumber) || 0) <= baselineRowNumber);
  await setMrnWhatsappProcessedKeys(baselineRecords, baselineRowNumber, spreadsheetId);
...
function mrnWhatsappParams(row = {}, actorName = "User", comment = "") {
  ...
  const materialSummary = (items = []) => {
    return items.length ? projectText(items.join("; "), 900).replace(/\s+/g, " ").trim() : "-";
  };
...
async function sendMrnWhatsappAutomation({ event = "actionRequest", row = {}, actor = null, comment = "" } = {}) {
  ...
  if (!settings.enabled) {
    const lastRun = await recordMrnWhatsappLastRun({ event: eventName, status: "disabled", mrnNo: row.mrnNo, reason: "Automation is disabled" });
    return { status: "disabled", sent: 0, failed: 0, results: [], lastRun };
  }
  const contactIds = event === "actionRequest" ? settings.approvalContactIds : settings.concernContactIds;
  ...
  if (event === "actionRequest" && !contacts.length) {
    contacts = allContacts.filter((contact) => contact.source === "manual" && normalizePhone(contact.phone));
  }
  const templateName = settings.templates?.[event] || MRN_WHATSAPP_DEFAULTS.templates[event];
  ...
  const lastRun = await recordMrnWhatsappLastRun({ event: eventName, status, sent, failed, recipients: contacts.length, templateName, language, mrnNo: row.mrnNo, results });
...
async function checkMrnWhatsappAutomationQueue({ source = "watcher" } = {}) {
  ...
  try {
    ...
    const processed = new Set([
      ...(settings.processedActionRequestMrns || []).map((item) => mrnWhatsappKey(item)).filter(Boolean),
      ...mrnWhatsappRuntimeProcessedKeys,
    ]);
    ...
    if (
      ...
      baselineRowNumber = await initializeMrnWhatsappWatcherFromSheet(records, activeSpreadsheetId);
    ...
    const pending = records
      ...
      .filter((row) => {
        if (initializedThisRun) return true;
        return !processed.has(mrnWhatsappRecordKey(row, activeSpreadsheetId)) && !processed.has(mrnWhatsappKey(row.mrnNo));
      })
  ...
  const run = () => {
    checkMrnWhatsappAutomationQueue({ source: "watcher" })
      .then((result) => {
        ...
        console.log(`MRN WhatsApp watcher ${result.status}: ${result.pending || 0} pending, ${result.sent || 0} sent, ${result.failed || 0} failed, checked ${result.checked || 0}${reason}`);
      })
...
function parseMrnWhatsappAction(message = {}) {
  ...
  const labelMatch = text.match(/\b(mrn\d+)\b/i);
  const mrnNo = labelMatch?.[1] || "";
  if (/approve/.test(text)) return { event: "approved", mrnNo, comment: "" };
...
async function handleMrnWhatsappReply(message = {}) {
  ...
  const dashboard = await readMrnDashboard({ all: true });
  const records = dashboard.records || [];
  const row = action.mrnNo
    ? records.find((record) => projectText(record.mrnNo).toLowerCase() === action.mrnNo.toLowerCase())
    : records[0];
  if (!row) {
    await recordMrnWhatsappLastRun({ event: action.event, status: "failed", failed: 1, reason: `Could not find MRN ${action.mrnNo || "record"} from WhatsApp reply` });
    return null;
  }
  ...
  const result = await sendMrnWhatsappAutomation({
    ...
    row,
    actor,
    comment: action.comment || projectText(message.text),
  });
  addActivityLog({
    action: "Sent MRN WhatsApp outcome",
    target: row.mrnNo || action.mrnNo || "MRN",
    category: "whatsapp",
    details: result.lastRun || result,
  });
  return result;
}
...
app.get("/mrn-dashboard/whatsapp/settings", requireSuperAdmin, async (req, res) => {
  try {
    res.json({
      settings: await getMrnWhatsappSettings(),
      contacts: whatsappService.listContacts().map(serializeWhatsappContact).filter((contact) => contact.phone),
    });
  } catch (error) {
    res.status(500).json({ error: error.message || "Could not load MRN WhatsApp settings" });
  }
});

app.patch("/mrn-dashboard/whatsapp/settings", requireSuperAdmin, async (req, res) => {
  try {
    const settings = await saveMrnWhatsappSettings(req.body || {});
    addActivityLog({ req, action: "Updated MRN WhatsApp automation", target: "MRN WhatsApp" });
    res.json({ success: true, settings });
  } catch (error) {
    res.status(400).json({ error: error.message || "Could not save MRN WhatsApp settings" });
  }
});

app.post("/mrn-dashboard/whatsapp/send-test", requireSuperAdmin, async (req, res) => {
  try {
    if (req.body?.settings) await saveMrnWhatsappSettings(req.body.settings);
    const dashboard = await readMrnDashboard({ all: true });
    const requestedMrn = projectText(req.body?.mrnNo);
    const row = requestedMrn
      ? (dashboard.records || []).find((record) => projectText(record.mrnNo).toLowerCase() === requestedMrn.toLowerCase())
      : (dashboard.records || [])[0];
    if (!row) return res.status(400).json({ error: "No MRN record found to send as test" });
    const result = await sendMrnWhatsappAutomation({
      event: req.body?.event || "actionRequest",
      row,
      actor: req.authUser,
...
app.post("/mrn-dashboard", upload.fields([
  ...
  try {
    ...
    let whatsapp = null;
    try {
      whatsapp = await sendMrnWhatsappAutomation({
        event: "actionRequest",
        row: result.snapshot,
        actor: req.authUser,
      });
...
async function buildForumProjectAssistantContext(project = {}) {
  ...
  return {
    ...
    project: {
      ...
      startDate: project.startDate || "",
      targetDate: project.targetDate || "",
      dueDate: project.targetDate || project.dueDate || project.deadline || project.endDate || "",
      location: project.location || "",
    },
