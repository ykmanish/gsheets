const SHEET_ID = "1LHeM2OI0d66oGFzjP_OnESDuuMPBm2AqpCouFBgB-DA";
const KIND = "aurika-client-dl";
...
function prepareSheet(name, values = []) {
  if (/data segregation room/i.test(name) || /catogory wise complete/i.test(name) || /category wise complete/i.test(name)) {
    return {
...
function apply(architecture, sheetData = []) {
  ...
  return {
    ...architecture,
    kind: KIND,
    displayName: "Aurika Client Design & Cost Dashboard",
    summary: {
      ...architecture.summary,
      purpose: "Give leadership one view of the Aurika design estimate, room and category costs, financial commitments, and scope decisions.",
