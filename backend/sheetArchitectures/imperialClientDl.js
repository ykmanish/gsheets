const SHEET_ID = "1JFOsTDtqHtE3pC-F-NjI-iDGf1XqGNB_iR514kDnLJE";
const KIND = "imperial-client-dl";
...
function prepareSheet(name, values = []) {
  ...
  if (/data roomwise/i.test(name) || /data/i.test(name)) {
    return {
...
function apply(architecture, sheetData = []) {
  ...
  return {
    ...architecture,
    kind: KIND,
    displayName: "Imperial Park Client Design & Cost Dashboard",
    summary: {
      ...architecture.summary,
      purpose: "Give leadership one view of the Imperial Park design estimate, room and category costs, financial commitments, and scope decisions.",
