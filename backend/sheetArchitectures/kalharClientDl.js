const SHEET_ID = "18tr8N8R7ja1awOs159wNLSbulNKsXYjULaVp0WPQQwU";
const KIND = "kalhar-client-dl";
...
function prepareSheet(name, values = []) {
  if (/data segregation room/i.test(name)) {
    return {
  ...
  if (/^summary$/i.test(name)) {
    ...
    values.forEach((row, index) => {
      ...
      const area = text(row[2]);
      const amount = row[8];
      if (index > 10 && area && !/^area$/i.test(area) && text(amount)) rows.push(["Interior budget", area, amount, ""]);
    });
    return { headers: ["Section", "Label", "Amount", "Notes"], rows, firstDataRow: 1, meta: { role: "executive-summary" } };
  }

  if (/^sheet33$|^purchase/i.test(name)) {
  ...
  if (/working for client|^index/i.test(name)) {
  ...
  if (headerRowIndex >= 0) {
    return {
      headers: uniqueHeaders(values[headerRowIndex]),
...
function apply(architecture, sheetData = []) {
  ...
  return {
    ...architecture,
    kind: KIND,
    displayName: "Kalhar Client DL Dashboard",
    summary: {
      ...architecture.summary,
      purpose: "Give leadership one view of the Kalhar design estimate, room and category costs, financial commitments, and scope decisions.",
