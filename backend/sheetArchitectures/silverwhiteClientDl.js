const SHEET_ID = "1111VZonNZijtDFPtO4ec_74WvMskfOii3bJEpcKdO9g";
const KIND = "silverwhite-client-dl";
...
function prepareSheet(name, values = []) {
  ...
  if (/data segregation room and catogory/i.test(name) || /data segregation/i.test(name) || /^data$/i.test(name)) {
    return {
  ...
  if (/sheet37|sheet33|purchase|pmt summary/i.test(name)) {
...
function apply(architecture, sheetData = []) {
  ...
  return {
    ...architecture,
    kind: KIND,
    displayName: "Silver White Client Design & Cost Dashboard",
    summary: {
      ...architecture.summary,
      purpose: "Give leadership one view of the Silver White design estimate, room and category costs, financial commitments, and scope decisions.",
