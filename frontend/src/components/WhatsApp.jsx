...
function Modal({ darkMode, title, onClose, children }) {
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div
        className={`max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-[28px] border p-5 -2xl sm:p-6 ${
          darkMode
            ? "border-white/10 bg-[#15171c] text-white"
      ...
      {groupModal && (
        ...
        >
          <form onSubmit={saveGroup}>
            ...
            <p className={`mb-3 mt-5 text-sm ${softText}`}>
              Select recipients
            </p>
