import { ArrowUpRight, Banknote, CalendarDays, CheckCircle2, FileText, Laptop, MapPin, Package, ShoppingCart, Table2, UserRound } from "lucide-react";
...
export default function AssetPurchaseDashboard({ darkMode, currentSheet, searchQuery, dateRange = {} }) {
  ...
  const requestedAmount = rows.reduce((sum, row) => sum + parseAmount(row.Amount), 0);
  const purchaseAmount = rows.reduce((sum, row) => sum + parseAmount(row["Purchase Amt"]), 0);
  const attachmentCount = rows.reduce((sum, row) => sum + extractUrls(row["Invoice and Warranty card"]).length, 0);
  const latest = sortedRows[0]?.Timestamp ? parseDate(sortedRows[0].Timestamp) : null;

  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6">
      <div className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <Metric darkMode={darkMode} icon={ShoppingCart} label={query ? "Matching requests" : "Requests"} value={rows.length.toLocaleString()} accent="#22c55e" />
        <Metric darkMode={darkMode} icon={Banknote} label="Requested amount" value={money(requestedAmount)} accent="#3b82f6" />
        <Metric darkMode={darkMode} icon={CheckCircle2} label="Purchase amount" value={purchaseAmount ? money(purchaseAmount) : "N/A"} accent="#f59e0b" />
        <Metric darkMode={darkMode} icon={FileText} label="Invoices / warranty" value={attachmentCount.toLocaleString()} accent="#ef4444" />
      ...
      <div className="mb-5 grid gap-4 xl:grid-cols-2">
        <Bars darkMode={darkMode} title="Amount by asset category" data={sumBy(rows, "Asset Category", "Amount")} valueFormatter={money} />
        <Bars darkMode={darkMode} title="Requests by category" data={countBy(rows, "Asset Category")} />
        <Bars darkMode={darkMode} title="Requests by employee" data={countBy(rows, "Employee Name")} />
        <Bars darkMode={darkMode} title="Payment mode split" data={countBy(rows, "Mode of Payment")} />
      </div>

      <div className="mb-5 grid gap-4 xl:grid-cols-2">
        {sortedRows.map((row) => {
          const attachments = extractUrls(row["Invoice and Warranty card"]);
          const amount = parseAmount(row["Purchase Amt"]) || parseAmount(row.Amount);
          return (
            <article key={`${row.__sheetName}-${row.__rowIndex}`} className={`rounded-[24px] p-5 ${darkMode ? "bg-white/[0.04] border border-white/10" : "bg-white border border-black/5"}`}>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <p className={`text-xl font-semibold ${darkMode ? "text-white" : "text-black"}`}>{cleanValue(row["Asset Name / Description"]) || "Asset request"}</p>
                  <p className={`mt-1 text-sm ${muted}`}>{cleanValue(row["Purpose / Requirement"]) || cleanValue(row["Vendor Name and Address"]) || "No purpose added"}</p>
                </div>
                <span className={`rounded-full px-3 py-1.5 text-sm font-medium ${darkMode ? "bg-[#d8f36a] text-black" : "bg-black text-white"}`}>{amount ? money(amount) : "N/A"}</span>
              </div>

              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ["Employee", cleanValue(row["Employee Name"]), UserRound],
                  ["Category", cleanValue(row["Asset Category"]), Laptop],
                  ["Quantity", cleanValue(row.QTY), Package],
                  ["Location", cleanValue(row.Location), MapPin],
                  ["Vendor", cleanValue(row["Vendor Name and Address"]), FileText],
                  ["Payment mode", cleanValue(row["Mode of Payment"]), Banknote],
                  ["Status", cleanValue(row.Status), CheckCircle2],
                  ["Purchased date", parseDate(row["Purchased Date"])?.toLocaleDateString() || cleanValue(row["Purchased Date"]), CalendarDays],
              ...
              {cleanValue(row["warranty and guarantee Details"]) && (
                <div className={`mt-3 rounded-2xl p-3 ${darkMode ? "bg-black/20" : "bg-black/[0.025]"}`}>
                  <p className={`mb-1 text-[10px] uppercase tracking-[0.16em] ${darkMode ? "text-white/35" : "text-black/35"}`}>Warranty / guarantee</p>
                  <p className={`text-sm ${darkMode ? "text-white/75" : "text-black/70"}`}>{cleanValue(row["warranty and guarantee Details"])}</p>
                </div>
              )}
      ...
      <div className={`overflow-hidden rounded-[24px] ${darkMode ? "bg-white/[0.035] border border-white/10" : "bg-white border border-black/5"}`}>
        <div className="flex items-center justify-between border-b border-black/5 px-5 py-4 dark:border-white/10">
          <h3 className={`text-sm font-medium ${darkMode ? "text-white" : "text-black"}`}>Complete asset purchase data</h3>
              ...
              {sortedRows.slice(0, 150).map((row) => (
                <tr key={row.__rowIndex} className={darkMode ? "border-b border-white/5" : "border-b border-black/5"}>
                  {headers.map((header) => {
                    ...
                    return (
                      <td key={header} className={`min-w-[170px] px-5 py-3 align-top ${darkMode ? "text-white/75" : "text-black/65"}`}>
                        {urls.length > 0 ? (
