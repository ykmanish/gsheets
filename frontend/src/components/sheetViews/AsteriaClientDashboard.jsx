...
function ExecutiveOverview({ darkMode, sheets, query }) {
  const detailSheet = findSheet(sheets, /^DL 1$/i) || findSheet(sheets, /^DL$/i) || sheets.find((sheet) => sheet.headers.some((header) => /^particulars?$/i.test(header)));
  const summarySheet = findSheet(sheets, /^Summary$/i);
  const financeSheet = findSheet(sheets, /^Sheet33$/i);
  const scopeSheet = findSheet(sheets, /working for client/i);
  ...
  const estimate = detailRows.reduce((sum, row) => sum + parseAmount(firstValue(row, ["U&I Amount", "U&I Amt", "Client Amt"])), 0);
  ...
  const roomSpend = sumBy(detailRows, ["Area", "Room"], ["U&I Amount", "U&I Amt", "Client Amt"]);
  const categorySpend = sumBy(detailRows, ["Category", "Catogory"], ["U&I Amount", "U&I Amt", "Client Amt"]);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p className={`text-xs uppercase tracking-[0.2em] ${darkMode ? "text-[#d8f36a]" : "text-green-700"}`}>Leadership snapshot</p><h2 className={`mt-2 text-2xl font-semibold ${darkMode ? "text-white" : "text-black"}`}>Asteria design & cost overview</h2></div>
...
export default function AsteriaClientDashboard({ darkMode, currentSheet, sheets = [], overview = false, searchQuery = "" }) {
