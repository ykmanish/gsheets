...
function ExecutiveOverview({ darkMode, sheets, query }) {
  ...
  const financeSheet = findSheet(sheets, /^Sheet37$|^Sheet33$|^Purchase/i);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p className={`text-xs uppercase tracking-[0.2em] ${darkMode ? "text-[#d8f36a]" : "text-green-700"}`}>Leadership snapshot</p><h2 className={`mt-2 text-2xl font-semibold ${darkMode ? "text-white" : "text-black"}`}>Silver White design & cost overview</h2></div>
...
export default function SilverWhiteClientDashboard({ darkMode, currentSheet, sheets = [], overview = false, searchQuery = "" }) {
